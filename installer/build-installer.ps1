...
dotnet publish $appProject -c Release -r win-x64 --self-contained false --artifacts-path $artifacts -o $payload
if ($LASTEXITCODE -ne 0) { throw 'ZFTP app publish failed.' }

Write-Host 'Publishing updater...' -ForegroundColor Cyan
dotnet publish $updaterProject -c Release -r win-x64 --self-contained false --artifacts-path $artifacts -o $payload
